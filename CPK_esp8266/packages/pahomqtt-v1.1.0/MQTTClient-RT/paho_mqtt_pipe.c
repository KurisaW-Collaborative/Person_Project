...
#include <ipc/pipe.h>  // 修改 <pipe.h> 为 <ipc/pipe.h>
                       // 使能 components -> c/C++ -> posix -> IPC -> pipe(进程间通信管道)
                       // paho_mqtt.h 内的  MQTTClient 结构体的代码，94 行改为 #if defined(RT_USING_DFS_NET) || defined(SAL_USING_POSIX)；去掉关于 RT_USING_POSIX 的内容
